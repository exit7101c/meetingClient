    ...
    >
      <swiper-slide v-for="(item, index) in itemList" :key="index">
        <ion-card @click="goOpenChatView(item.openChatKey)" mode="md">
          ...
          <ion-text class="icon-star">
            <ion-text color="primary">{{ item.cntBookmark }}</ion-text>
            <ion-icon color="primary" :icon="star" size="small" v-if="item.bookmark == 'Y'" />
            <ion-icon color="light" :icon="star" size="small" v-else style="" />
          </ion-text>
          <ion-card-header>
            <!-- TODO: 소모임 분류 -->
            <ion-chip color="light" outline class="shape-round">{{ item.partitionNm }}</ion-chip>
            <ion-card-title>{{ item.title }}</ion-card-title>
            <ion-card-subtitle>
              <!-- TODO:  소모임 지역 & 인원 수-->
              <span>
                <ion-icon :icon="locationSharp" size="small" />
                <ion-text v-if="item.latlonDistance != null">{{ item.latlonDistance }}km</ion-text>
                <ion-text v-else>장소미정</ion-text>
              </span>
              <span>
                <ion-icon :icon="personSharp" size="small" />
                <ion-text>{{ item.cntUser }}</ion-text>
              </span>
            </ion-card-subtitle>
          </ion-card-header>
        </ion-card>
