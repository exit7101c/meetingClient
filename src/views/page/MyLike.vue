      ...
      <div class="mt-2 pt-2">
        ...
        >
          <template #content>
            ...
            <br />
            <ion-text color="light" class="text-lg">
              <!--{{freeFlipInfo}} -->받은 좋아요 확인하기 잔여수량 {{ freeFlipCnt }} / 1
            </ion-text>
            <br />
          </template>
        </MyLikeTopNotice>
  ...
  methods: {
    ...
    getUserReceiveLikeCnt() {
      getData({
        ...
        then: (data) => {
          ...
          this.freeFlipInfo = data.freeFlipInfo;
          this.freeFlipCnt = data.freeFlipCnt;
        }
      });
    },
    getReceiveLike() {
    ...
    dailyCardDetail(item) {
      ...
      if (item.superLikeYn == "Y") {
        ...
        this.$router.push("/dailyCardDetail");
      } else if (item.freeFlipYn == "Y") {
        // 무료 받은 좋아요 확인하기 를 사용 한 경우
        dailyCardInfoMapFn({ userKey: item.userKey, pageType: "myLike", likeSeq: item.likeSeq, nick: item.nick });
        this.$router.push("/dailyCardDetail");
      } else {
        // 무료 받은 좋아요 확인하기가 사용 가능한지 조회
        getData({
          url: "/getFreeFlipIsPossible",
          param: {},
          then: (data) => {
            if(data.successYn == 'Y'){
              // 사용 가능 할 경우 사용 Alert
              this.freeFlipUsingAlert(item);
            } else {
              // 사용이 불가능 할 경우 구독 Alert
              // 구독아니라고 띄워주기
    ...
    /** 무료 받은 좋아요 확인하기 사용 팝업창 **/
    async freeFlipUsingAlert(item) {
      const alert = await alertController.create({
        header: "",
        subHeader: "(00시~08시/08시~16시/16시~24시 시간별 1회)",
        message: "현재 받은 좋아요 확인하기가 1회 사용 가능합니다. 사용하시겠습니까?",
        buttons: [
          {
            text: "취소",
            role: "cancel",
          ...
          },
          {
            text: "사용",
            handler: () => {
              // 무료 받은 좋아요 확인하기 사용 로직
              getData({
                url: "/setFreeFlip",
                param: {likeSeq: item.likeSeq},
                then: (data) => {
                  if(data.successYn == 'Y'){
                    item.freeFlipYn = 'Y';
                    this.freeFlipCnt = 0;
