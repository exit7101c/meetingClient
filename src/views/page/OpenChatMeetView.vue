        ...
        <div>
          <div class="row-box"
            ...
            <ion-buttons slot="end" v-if="BoardDetail.isCreatedUser == 'Y'">
              <custom-button id="openChatMeetView-popover-button">
                <ion-icon slot="icon-only" :icon="menuIcon" color="light" />
              </custom-button>
              <ion-popover
                trigger="openChatMeetView-popover-button"
          ...
          <div class="row-box">
            ...
            <p
              @mousedown="startTimer" @mouseup="clearTimer"
              @touchstart="startTimer" @touchend="clearTimer"
            >
...
export default {
  ...
  methods: {
    ...
    async copyClipboard() {
      try {
        await Clipboard.write({
          string: this.BoardDetail.replaceContent
        });
        await this.warningAlert("텍스트가 클립보드에 복사되었습니다.");
      } catch (err) {
        await this.warningAlert("텍스트를 클립보드에 복사하는데 실패했습니다.", err);
      }
    },
    ...
    goOpenChatMeetWrite() {
      console.log("dasdasd");
      console.log(openChatMeetViewMap);
