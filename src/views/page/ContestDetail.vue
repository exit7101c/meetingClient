  ...
  <ion-page>
    <ion-header>
      <ion-toolbar>
        ...
        <ion-title>{{ BoardDetail.nick }}님</ion-title>
        ...
        <div>
          <div class="row-box">
            ...
            >
              ...
              <swiper-slide v-if="this.attr2 !== null && this.attr2 !== ''">
                <ion-img
              ...
              <swiper-slide v-if="this.attr3 !== null && this.attr3 !== ''">
                <ion-img
              ...
              <swiper-slide v-if="this.attr4 !== null && this.attr4 !== ''">
                <ion-img
                  v-if="this.attr4 !== null"
                  v-bind:src="'https://' + attr4"
                  @ionError="$event.srcElement.src = require('../../assets/img/Loading_icon.gif')"
                />
              </swiper-slide>
            </swiper>
          ...
          <div class="row-box">
            <CardBox padding="none" class="my-point-info" @click="voteHis">
              <ion-item lines="none" style="left: 25%;">
                <ion-icon
                  slot="start"
                  :icon="voteIcon"
                  color="light"
                ...
                >투표 수 <strong class="text-bold">{{ BoardDetail.votesCnt }}</strong
                > 표
                </ion-text>
              </ion-item>
            </CardBox>
          ...
          <div class="row-box">
            <custom-button
              ...
              @click="useContestVote(checkDisabled = false)"
            >
              <span>투표하기</span>
              <span v-if="coupon.quantity > 0" style="margin-left: 5px;"> X {{ coupon.quantity }}</span>
...
import {
  ...
  communityViewMap,
  communityWriteMapFn,
  policeMapFn, messageTalkMapFn, communityViewMapFn, contestDetailMapFn, dailyCardInfoMap, partnerInfoMapFn
...
export default {
  ...
  methods: {
    btnType(state) {
      if (state === "p") {
        policeMapFn({
          targetKey: communityViewMap.article,
          type: "contestDetail",
          item: this.BoardDetail,
          targetId: communityViewMap.targetUserKey
        });
        this.$router.push("/police");
      } else {
        this.presentAlertConfirm_blockUser();
      }
    },
    voteHis() {
      contestDetailMapFn({ targetUserId: this.BoardDetail.targetUserId });
      this.$router.push("/contestVoteHis");
    },
    limitMessage(e) {
    ...
    useContestVote() {
      getData({
        ...
        param: {
          ...
          targetUserKey: communityViewMap.targetUserKey // 선택한 userId
        },
        then: (data) => {
          if (data.successYn === "NH") {
            this.warningAlertNH(data.message);
          } else if (data.successYn == "NI") {
            this.warningConfirm(data.message, () => {
              this.$router.push("/store");
            });
          } else if (data.successYn == "N_START") {
    ...
    getBoardDetail(isScroll = false) {
      ...
      getData({
        ...
        then: (data) => {
          ...
          this.attr4 = this.BoardDetail.attr4CdnNm;
          ...
          if (this.commentList.length > 0) {
            ...
            this.lastCommentKey = this.commentList[this.commentList.length - 1].commentKey;
          }
        }
      });
    },
      ...
      setTimeout(function() {
        ...
        if (self.searchType == "second") {
          getData({
            ...
            param: {
              articleKey: communityViewMap.article
              , lastCommentKey: self.lastCommentKey
            },
            then: (data) => {
              ...
              self.$nextTick(() => {
                ...
                self.lastCommentKey = self.commentList[self.commentList.length - 1].commentKey;
    ...
    async presentAlertConfirm_blockUser() {
      const alert = await this.alertController.create({
        // cssClass: 'my-custom-class',
        header: "( " + this.BoardDetail.nick + " ) 님을 차단하시겠습니까?",
        message: "해당유저를 일부 화면에 노출시키지 않습니다.",
        buttons: [
          {
            text: "취소",
            role: "cancel",
          ...
          },
          {
            ...
            handler: () => {
              ...
              getData({
                url: "/blockByUserInfo",
                param: { targetUserKey: communityViewMap.targetUserKey },
                then: (data) => {
                  this.warningAlert(data.message);
                }
              });
