<template>
  <ion-page>
    <custom-header page-name="게시글"></custom-header>
    <ion-content ref="communityContainer">
      ...
      <div class="sticky-top">
        <!--  탭   -->
        <ion-segment v-model="categoryCd" mode="md" :scrollable="true" value="">
          <ion-segment-button value="" @click="categoryBtn('all')">
          ...
          >
            <ion-text class="text-bold">{{ item.boardNm }}</ion-text>
          </ion-segment-button>
        </ion-segment>
    ...
    <!-- TODO : 검색 모달 -->
    <ion-modal
      :is-open="$store.state.communitySearchModalOpen"
      @ionModalDidPresent="searchOpen"
      :initial-breakpoint=".5"
      :breakpoints="[0, .5, 1]"
      @ionModalDidDismiss="$store.state.communitySearchModalOpen = false"
    >
      <ion-content>
          ...
          <template #content>
            ...
            <div class="row-box">
              ...
              >
                ...
                <div>
                  <ion-text color="secondary" class="text-sm" @click="$store.state.communitySearchModalOpen = false">닫기
                  </ion-text>
                </div>
              </ion-row>
            </div>
    ...
    getBoardList() {
      ...
      getData({
        ...
        then: (data) => {
          ...
          this.setLocalSearchAfterHide();
          this.$store.state.communitySearchModalOpen = false;
        }
      });
    },
    ...
    setLocalSearchAfterHide() {
      ...
      for (let idx in localStorageBlockList1) {
        for (let idx2 in this.BoardList)
          if (
            localStorageBlockList1[idx].articleKey ==
            this.BoardList[idx2].articleKey
          ) {
