<template>
  <ion-page>
    ...
    <ion-content class="ion-padding">
        ...
        <ion-list>
          ...
          >
            <ion-textarea
              ...
              placeholder="우리 모임을 자세하게 소개해주세요."
            />
          </ion-item>
        </ion-list>
      </div>
      <!--  TODO : 주제 선택   -->
      <div class="row-box">
        <ion-text color="light">주제 선택</ion-text>
        <div class="row-box">
          <ion-segment
            v-model="partitionCd"
            mode="ios"
            :scrollable="true"
            class="segment-chip auto-width pl-0"
          >
            <ion-segment-button
      ...
      <!--  TODO : 서브 주제 선택   -->
      <div class="row-box" v-if="partitionSubList.length > 0">
        <ion-text color="light">서브 주제 선택</ion-text>
        <div class="row-box">
          <ion-segment
          ...
          >
            <ion-segment-button
              v-for="(item, idx) in partitionSubList"
              :key="idx"
      ...
      <!--<div class="row-box">
        ...
        >
          ...
          <ion-button size="small" color="primary" shape="round">
            {{ peopleNumber > 0 ? peopleNumber + '명' : '선택' }}
          </ion-button>
        </ion-row>
      </div>-->
      <!--   하단 시간 및 장소 -->
        ...
        <ion-item class="activateCheckList-item" lines="none">
          ...
          <ion-label class="text-left">
            <ion-text class="activeTimeText" readonly>
              다른 사용자들이 모임 정보에서 마지막 활동 시간을 볼 수 있어요 !
            </ion-text>
          </ion-label>
        </ion-item>
        <ion-item class="activateCheckList-item" lines="none">
  ...
  methods: {
    ...
    async confirmBtn(type) {
      ...
      const alert = await this.alertController.create({
        ...
        header: "",
        message: type == "edit" ? "입력한 내용대로 수정 할까요?" : "채팅방을 생성합니다.",
        buttons: [
          {
            text: "취소",
            role: "cancel",
