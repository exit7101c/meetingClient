<template>
  <ion-page>
    <custom-header page-name="초이스" class="main-header"></custom-header>
    <ion-content :fullscreen="true">
      ...
      >
        ...
        <ion-row>
          <ion-col size="12">
            <ion-row class="gap-lg">
              <ion-text color="light">제휴업소 광고 제거</ion-text>
              <NavyPlusLabel />
            </ion-row>
            <p class="mt-1">
              <ion-text color="light" class="text-sm sub-text04"
              >제휴업소 광고를 더이상 보지 않습니다
              </ion-text
              >
              <ion-text color="light" class="text-sm sub-text04"
              >일반 광고는 구독을 하면 제거 됩니다.
              </ion-text
              >
            </p>
          </ion-col>
...
import { getData, storeMapFn, showAdPop } from "@/assets/js/common";
...
export default {
  ...
  methods: {
    ...
    buttonClick(type, item) {
      if (type === "nope") {
        ...
        if (this.choiceYn == "Y") {
          ...
          this.warningAlert("프로필이 완성되지 않았어요! 호감을 표시하려면 프로필설정을 완료해주세요. (필수사진등록)");
        }
    ...
    reachEnd() {
      ...
      //새로운 사용자카드 가져올때 광고 호출
      //구독하지 않은 사용자만
      /*if (this.subscribeInfo.subscribeYn == "N") {
        showAdPop();
      }*/
