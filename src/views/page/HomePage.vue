<template>
  <ion-page>
    <custom-header page-name="라운지"></custom-header>
      ...
      <div class="ion-padding main-content">
        ...
        <MainSection>
          ...
          <template #content>
            <!-- TODO: 핫피플 카테고리 -->
            <ion-segment
              v-model="hotpeopleStateCd"
              mode="ios"
              :scrollable="true"
              class="segment-chip auto-width pt-0 pb-0 pl-0"
            >
              <ion-segment-button
                v-for=" season in hotPeopleStateList"
                :value="season.stateCd"
                :key="season.stateCd"
                class="shape-round"
                @click="categoryBtn(season, 'season')"
              >{{ season.stateNm }}
              </ion-segment-button>
              <ion-segment-button
        ...
        <MainSection>
          <template #title>
            <MainSectionTitle
              title="ASK ME"
              subTitle="오늘 가장 많이 평가를 받은사람은 누구?"
              color="#ffffff"
              :icon="askMeIcon"
            >
            </MainSectionTitle>
          ...
          <template #content>
            <div class="row-box" v-if="hotAskMeList.length > 0">
              <MainHotPeopleSlider
                :item-list="hotAskMeList"
                :loaded="!false"
              />
            </div>
            <div class="row-box" v-else>
              <div
                ...
                style="padding: 0; margin-bottom: 20px; color: white"
              >
...
export default {
  ...
  data() {
    return {
      ...
      // TODO : 카테고리 핫피플
      hotPeopleStateList: [],
      hotpeopleStateCd: "today",
      hotpeopleCateCd: "today",
      hotpeopleCateList: [
        // {
        //   cateCd: "season",
  ...
  methods: {
    ...
    categoryBtn(item, type) {
      if (type == "hot") {
        ...
        this.hotpeopleCateCd = "season";
        this.hotpeopleStateCd = item.stateCd;
        this.hotpeopleCategory();
      } else if (type == "openChat") {
    ...
    hotpeopleCategory() {
      getData({
        ...
        param: {
          category: this.hotpeopleCateCd,
          stateCd: this.hotpeopleStateCd
        },
        then: (data) => {
    ...
    getHotpeopleState() {
      getData({
        url: "/getHotpeopleState",
        param: {},
        then: (data) => {
          this.hotPeopleStateList = data;

          if (this.hotPeopleStateList.length > 0) {
            this.hotpeopleCateCd = "season";
            this.hotpeopleStateCd = data[0].stateCd;
          }
