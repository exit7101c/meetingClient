<template>
  <ion-modal :is-open="isOpen" @ionModalDidDismiss="ionModalDidDismiss">
    <ion-toolbar> 회원탈퇴</ion-toolbar>
    <ion-content>
      <ion-content>
        <div class="message">
          <p>
            <!--            ※ 회원탈퇴 시 개인정보 및 NAVY에서 만들어진 모든 데이터는-->
            ※ 회원탈퇴 시 NAVY에서 만들어진 데이터는 삭제됩니다.<br /><br />

            1. 회원은 언제든지 회원탈퇴(계정삭제)를 할 수 있습니다.<br />
            <!--            2. 회원탈퇴 시, 계정정보는 복구되지 않으며 회원의 모든 정보는 즉시-->
            <!--            파기됩니다.<br />-->
            2. 회원탈퇴 시 회원이 작성한 게시물은 삭제되지 아니하며, 당사의 관리
            목적으로 보관됩니다.<br />
            3. 당사는 회원이 신고처리규정 5단계(영구정지)에 준하는 행위를 하였을
            경우 사전통지 없이 이용권한을 제한할 수 있습니다.
          </p>
        </div>
        <div class="modal-footer">
          <ion-item lines="none" class="custom-checkBox">
            <ion-checkbox mode="ios" slot="start" v-model="userSecess">
            </ion-checkbox>
          ...
          <ion-row>
            <ion-col style="margin: 0 auto; align-items: center">
              ...
              <custom-button shape="round" @click="handleUserSecession">
                탈퇴하기
              </custom-button>
            </ion-col>
          </ion-row>
