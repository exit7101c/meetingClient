          ...
          >
            <ion-content>
              <ion-list lines="full">
                ...
                <template v-if="SS_USER_KEY == 'oicc8XQmIB8SklPssUoYtg==' && BoardDetail.appTesterYn == 'Y'">
                  <ion-item
                    :button="true"
                    :detail="false"
                    @click="adminHideCommunity()"
                  >관리자숨김
                  </ion-item>
                </template>
                <template v-else-if="SS_USER_KEY == 'oicc8XQmIB8SklPssUoYtg==' && BoardDetail.appTesterYn == 'N'">
                  <ion-item
                    :button="true"
                    :detail="false"
                    @click="adminHideCommunity()"
          ...
          <div class="row-box">
            <p>
              <ion-text
                color="light"
                class="text-md"
                v-html="BoardDetail.replaceContent"
              ></ion-text>
            </p>
          </div>
          <!--  추천 버튼   -->
          ...
          <CommentItem v-for="(data, index) in commentList" :key="index">
            ...
            <template #content>
              <CommentContent
                ...
                @hideComment="setHide('comment', data)"
              />
              <!-- 댓글 수정 폼-->
              <div
                class="row-box"
              ...
              >
                ...
                <template #content>
                  <CommentContent
                    ...
                    @hideComment="setHide('comment', data2)"
                  />
                  <!-- 댓글 수정 -->
                  <div
                    class="row-box"
...
export default {
  ...
  ionViewWillEnter() {
    ...
    if (this.$route.query.isFromFeedYn === "Y") {
      this.$nextTick(() => {
        this.scrollToBottom();
      });
    }
  ...
  methods: {
    dropCheck(type) {
      if (communityViewMap.article) {
        ...
        this.warningAlert(
          "게시글 정보를 불러오는데 실패했습니다.<br />다시 시도해 주세요."
        );
        this.$router.push("/community");
        return;
      }
    ...
    limitComment(e) {
      ...
      if (rows > 20) {
        if (e.inputType !== "deleteContentBackward") {
          ...
          this.warningAlert("줄바꿈은 20줄까지만 가능합니다");
          return;
        }
      }
    ...
    replaceContentURL() {
      ...
      Array.from(match).forEach((item) => {
        if (!item.includes("http")) {
          this.BoardDetail.replaceContent =
            this.BoardDetail.replaceContent.replaceAll(
              item,
    ...
    replaceCommentURL() {
      ...
      Array.from(this.commentList).forEach((comment, index) => {
        ...
        Array.from(match).forEach((item) => {
          if (!item.includes("http")) {
            this.commentList[index]["replaceContents"] = this.commentList[
              index
              ]["replaceContents"].replaceAll(
              item,
              `<a href="http://${item}" target="_blank">${item}</a>`
            );
          } else {
        ...
        if (comment.childCommentList.length > 0) {
          Array.from(comment.childCommentList).forEach(
            (childComment, commentIndex) => {
              let _commentMatch = childComment.contents.match(regexp);
              let commentMatch = [...new Set(_commentMatch)];
              this.commentList[index]["childCommentList"][commentIndex][
                "replaceContents"
    ...
    setHide(type, item) {
      ...
      if (type == "title") {
        ...
        localStorageBlockList = JSON.parse(
          localStorage.getItem("commentBlockList")
        );
      }
      ...
      if (type == "title") {
        ...
        for (let idx in localStorageBlockList) {
          if (localStorageBlockList[idx]["commentKey"] == item.commentKey) {
            isExist = true;
          }
        }
      }
      ...
      if (isExist) {
        //숨김해제
        this.blockCancelConfirmBtn(type, item);
      } else {
        //숨김
        this.blockConfirmBtn(type, item);
      }
    },
    async blockCancelConfirmBtn(type, item) {
      const alert = await this.alertController.create({
        //cssClass: 'my-custom-class',
        header: "",
          ...
          },
          {
            ...
            handler: () => {
              ...
              if (type == "title") {
                ...
                for (let i in localStorageBlockList) {
                  if (
                    localStorageBlockList[i]["commentKey"] == item.commentKey
                  ) {
                    localStorageBlockList.splice(i, 1);
                  }
                }
    ...
    async blockConfirmBtn(type, item) {
      let confirmMessage = "";
      if (type == "title") {
        confirmMessage =
          "이 게시글 제목을 더이상 보지 않습니다. 계속 하시겠습니까?";
      } else if (type == "comment") {
        confirmMessage =
          "이 댓글 내용을 더이상 보지 않습니다. 계속 하시겠습니까?";
      }
          ...
          },
          {
            ...
            handler: () => {
              ...
              if (type == "title") {
                ...
                item.replaceContent = "사용자에 의해 숨김처리된 게시글입니다.";
                this.attr1 = null;
                this.attr2 = null;
                this.attr3 = null;
              } else if (type == "comment") {
                localStorageBlockList.push({ commentKey: item.commentKey });
                localStorage.setItem(
                  "commentBlockList",
                  JSON.stringify(localStorageBlockList)
                );
                item.replaceContents = "사용자에 의해 숨김처리된 댓글입니다.";
                item.cdnNm = null;
              }
    ...
    setLocalSearchAfterHide() {
      ...
      let localStorageBlockList2 = [];

      localStorageBlockList1 = JSON.parse(
        localStorage.getItem("articleBlockList")
      );
      localStorageBlockList2 = JSON.parse(
        localStorage.getItem("commentBlockList")
      );

      for (let idx in localStorageBlockList1) {
        if (
          localStorageBlockList1[idx].articleKey == this.BoardDetail.articleKey
        ) {
          this.BoardDetail.title = "사용자에 의해 숨김처리된 게시글입니다.";
          this.attr1 = null;
          this.attr2 = null;
          this.attr3 = null;
          this.BoardDetail.replaceContent =
            "사용자에 의해 숨김처리된 게시글입니다.";
          break;
        }
      }
      for (let idx in localStorageBlockList2) {
        for (let idx2 in this.commentList) {
          if (
            this.commentList[idx2].childCommentList != undefined &&
            this.commentList[idx2].childCommentList.length > 0
          ) {
            for (let idx3 in this.commentList[idx2].childCommentList) {
              if (
                localStorageBlockList2[idx]["commentKey"] ==
                this.commentList[idx2].childCommentList[idx3].commentKey
              ) {
                this.commentList[idx2].childCommentList[idx3].replaceContents =
                  "사용자에 의해 숨김처리된 댓글입니다.";
                this.commentList[idx2].childCommentList[idx3].cdnNm = null;
              }
            }
          }
          if (
            localStorageBlockList2[idx]["commentKey"] ==
            this.commentList[idx2].commentKey
          ) {
            this.commentList[idx2].replaceContents =
              "사용자에 의해 숨김처리된 댓글입니다.";
            this.commentList[idx2].cdnNm = null;
          }
        }
      }
    },
    ...
    adminHideCommunity() {
      getData({
        ...
        param: {articleKey: communityViewMap.article},
        target: this,
        then: (data) => {
          this.warningAlert(data.message);
        }
