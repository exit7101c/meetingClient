  ...
  methods: {
    ...
    roomExit() {
      if (this.chatRoomLeader == 'Y' && this.leaderCanOutYn == 'Y') {
        ...
        // 해당 경우는 없어야 정상
        this.warningAlert(
          '소모임에서 방장은 혼자 남은 경우에만 나갈 수 있습니다.'
        );
      } else {
    ...
    send(type) {
      ...
      if (this.isChatBlock) {
        this.warningAlert(
          '삭제된 소모임입니다.<br />삭제된 글에는 더 이상 채팅을 할 수 없습니다.'
        );
        return;
      }
    ...
    useOpenChatBoost() {
      getData({
        ...
        then: (data) => {
          if (data.privateYn == 'N') {
            this.warningAlert(
              '비활성 상태인 모임에는 모임부스터를 <br />사용할 수 없습니다.'
            );
            return;
          } else {
            let meg = '사용하시겠습니까?';
    ...
    goRouter(type, item) {
      if (type === 'profile') {
        ...
        if (item.subLeaderYn === 'Y') {
          this.warningAlert(
            '부방장은 강퇴할수없습니다. 부방장에서 해제시킨후 강퇴할수있습니다.'
          );
          return;
        }
    ...
    async blockCancelConfirmBtn(item) {
      const alert = await alertController.create({
        ...
        header: '',
        message: '이미 차단되어 있습니다. 차단을 해제 하시겠습니까?',
        buttons: [
          {
            text: '취소',
            role: 'cancel',
    ...
    async blockConfirmBtn(item) {
      const alert = await alertController.create({
        ...
        header: '',
        message:
          '이 사용자의 메시지를 채팅에서 더 이상 보지 않습니다. 계속 하시겠습니까?',
        buttons: [
          {
            text: '취소',
            role: 'cancel',
    ...
    async blockFinishBtn(item) {
      const alert = await alertController.create({
        //cssClass: 'my-custom-class',
        header: '',
        message: '사용자가 차단되어 메시지를 받지 않게됩니다.',
        buttons: [
          {
            text: '확인',
            role: 'cancel',
    ...
    async confirmBtn(item, type) {
      ...
      if (type === 'master') {
        ...
        message = '부방장으로 위임 하시겠습니까?';
        url = '/messageMain/setSubRoomMasterChange';
        btnText = '위임';
      } else if (type === 'subRemoval') {
        message = '부방장을 해제 하시겠습니까?';
