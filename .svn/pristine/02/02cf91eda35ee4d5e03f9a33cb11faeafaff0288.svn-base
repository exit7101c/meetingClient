<template>
  <ion-page>
    <custom-header page-name="설정"></custom-header>
    <ion-content class="ion-padding">
        ...
        <CardBox padding="none" class="my-point-info">
          <ion-list lines="none">
            ...
            <ion-item v-if="isPartner == 'Y'">
              ...
              <ion-text class="text-md" color="light">
                파트너 포인트
                <strong class="text-bold">{{ currentPoint }}</strong>
                P
              </ion-text>
              <ion-text
                slot="end"
      ...
      <div class="row-box">
        <MyPageCertification :myBadgeList="myBadgeList" />
      </div>
      <div class="row-box">
        <AskMeLink
          :askMeInfo="askMeInfo"
      ...
      <div class="row-box" v-if="isPartner == 'Y'">
        <ion-list lines="none">
          <MyPageNavButtonItem title="파트너스 관리자" @click="partners" />
          <!--          <MyPageNavButtonItem-->
          <!--            title="받은 쪽지 확인"-->
      ...
      <div class="row-box">
        <MyPageNavList
          @profileModalBtn="profileModalBtn('openModal')"
          :isPartner="isPartner"
        />
      </div>
...
import MyPageCertification from "@/components/MyPage/MyPageCertification.vue";
...
import { copyOutline, arrowForwardOutline } from "ionicons/icons";
  ...
  methods: {
              ...
              getData({
                ...
                then: (info) => {
                  if (info.successYn == "Y") {
                    ...
                    self.updDate = info.result.updDate;
                    self.$store.state.partnerInfoMap["partnersCode"] =
                      info.result.partnersCode;
                  }
                }
              });
            }
    ...
    async partnersRouteAlert() {
      ...
      const alert = await this.alertController.create({
        ...
        header: "",
        message:
          "파트너스 정보가 등록되지 않았습니다.<br />지금 등록하시겠습니까?",
        buttons: [
          {
            text: "아니요",
            role: "cancel",
